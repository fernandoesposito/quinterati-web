...
export default function Solutions() {
  ...
  const FilteredCardDetails = () => {
    ...
    useEffect(() => {
      const [ topic ] = cardsDescription?.[activeTopicId!]?.filter((card: CardProps) => card.id === selectedId);
  ...
  return (
    <section className="w-screen bg-main md:px-12 px-4 md:pt-[calc(5rem+32px)] max-md:pt-[calc(3.25rem+1.5rem)] pb-12 flex flex-col items-center">
      ...
      <div className="grid grid-cols-3 gap-6 w-fit mb-14 max-md:hidden">
        { cardsDescription?.[activeTopicId!]?.map((card: CardProps, index: number) => 
          <ProductCard 
      ...
      { selectedId !== null ? 
      <div className="mb-6 max-md:mt-8">
        <FilteredCardDetails />
      </div> 
      : <></>
      }
